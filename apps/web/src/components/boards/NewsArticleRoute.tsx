...
export async function newsArticleMetadata(id: string) {
...
export default async function NewsArticleRoute({ id }: { id: string }) {
  let item;
  let allNews;
  let seo;
  try {
  ...
  const relatedLinks = [
    ...
    ...series.filter(Boolean).map((row) => ({ href: `/tournaments/${row!.id}`, label: row!.name })),
  ];
