...
export default function SearchBar({ autoFocus = false, onDone }) {
  ...
  const groups = results
    ? [
        { key: 'players', label: 'Players', items: results.players, to: (r) => `/players/${r.id}` },
        { key: 'teams', label: 'Teams', items: results.teams, to: (r) => `/teams/${r.id}` },
        { key: 'matches', label: 'Matches', items: results.matches, to: (r) => `/matches/${r.id}` },
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-semibold text-mtext">
                      {group.key === 'matches'
                        ? `${item.teams.home.code} vs ${item.teams.away.code}`
                        : item.name}
                    </span>
                    <span className="block truncate text-xs text-stext">
                      {group.key === 'players'
                        ...
                        : group.key === 'teams'
                          ...
                          : group.key === 'matches'
                            ? `${item.tournamentName} • ${item.status}`
