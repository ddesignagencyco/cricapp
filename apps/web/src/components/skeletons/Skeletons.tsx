...
function AdminContentSkeletonTheme({ children }: { children: ReactNode }) {
  return (
    <SkeletonTheme
      baseColor="var(--admin-input-bg)"
      highlightColor="var(--admin-border)"
      borderRadius={4}
      duration={1.35}
    >
      {children}
    ...
    <AdminContentSkeletonTheme>
      <div className="space-y-5" aria-busy="true" aria-label="Loading">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="rounded-lg p-4" style={{ border: '1px solid var(--admin-border)', background: 'var(--admin-card)' }}>
              <Skeleton width="40%" height={10} />
              <Skeleton width="30%" height={24} className="mt-2" />
            </div>
          ))}
        </div>
        <AdminTableSkeleton />
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid var(--admin-border)', background: 'var(--admin-table-header)' }}>
              {Array.from({ length: cols }).map((_, i) => (
                <th key={i} className="px-4 py-3"><Skeleton width={64} height={10} /></th>
              ))}
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={r} style={{ borderBottom: '1px solid var(--admin-border)' }}>
                {Array.from({ length: cols }).map((_, c) => (
                  <td key={c} className="px-4 py-3">
                    <Skeleton width={c === 0 ? '70%' : '45%'} height={10} />
                  </td>
                ))}
              </tr>
            ))}
